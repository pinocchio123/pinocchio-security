package com.pinocchio.security.config;
    ...
    public MessageHandler handler() {
        return new MessageHandler() {
            ...
            public void handleMessage(Message<?> message) throws MessagingException {
                logger.info(message.getHeaders().get("mqtt_receivedTopic")+message.getPayload().toString());
                String msg = "$cmd=setalertor&result=succ";
                String[] array = message.getPayload().toString().split("&");
                msg = msg + "&" + array[2];
                mqttSender.sendToMqtt(message.getHeaders().get("mqtt_receivedTopic").toString(),msg);
            }
        };
    }
}
